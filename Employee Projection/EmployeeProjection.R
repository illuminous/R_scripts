#load directory
 path <- "C:/jh_RScripts/Employee Projection"

 setwd(path)

#load data
 empCounts <- read.csv("KootenaiTrainCountsEmployeeDataNoDate.csv")
 ...
 #library("ggplot2")
 ggplot(empCounts, aes(x= Available, y = Trains.per.day)) + geom_point(size = 2.5, color="navy") + xlab("Available Employees") + ylab("Trains Per Day") + ggtitle("Proof of Concept with 68 Observations: Available Employees vs Trains Per Day")
