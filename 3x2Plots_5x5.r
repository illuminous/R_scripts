...
FuelsWest<- read.table("Fuel_load_west_analysis_set_5x5_outliersRemoved.csv",header = TRUE, sep = ",", row.names= 1)
head(FuelsWest)
summary(FuelsWest)
...
litter<-plot(FuelsWest$FIA_Litter.kg.m.2, FuelsWest$fivex_Litter.kg.m.2, xlim = c(0,15), ylim= c(0,5), ylab = "LANDFIRE Litter (median kg/m -2)", xlab =" FIA Litter (kg/m -2)")
Litterlm <-lm(FuelsWest$FIA_Litter.kg.m.2~FuelsWest$fivex_Litter.kg.m.2)
abline(Litterlm)

duff<-plot(FuelsWest$FIA_Duff.kg.m.2, FuelsWest$fivex_Duff.kg.m.2, xlim = c(0,45), ylim= c(0,23), ylab = "LANDFIRE Duff (median kg/m -2)", xlab =" FIA Duff (kg/m -2)")
dufflm <- lm(FuelsWest$FIA_Duff.kg.m.2~FuelsWest$fivex_Duff.kg.m.2)
abline(dufflm)

onehour<-plot(FuelsWest$FIA_1.hour.kg.m.2, FuelsWest$fivex_1.hour.kg.m.2, xlim = c(0,.5), ylim= c(0,.5), ylab = "LANDFIRE 1 Hr Fuels (median kg/m -2)", xlab =" FIA 1 Hr Fuels (kg/m -2)")
onehourlm <- lm(FuelsWest$FIA_1.hour.kg.m.2~FuelsWest$fivex_1.hour.kg.m.2)
abline(onehourlm)

tenhour<-plot(FuelsWest$FIA_10.hour.kg.m.2, FuelsWest$fivex_10.hour.kg.m.2, xlim = c(0,5), ylim= c(0,1), ylab = "LANDFIRE 10 Hr Fuels (median kg/m -2)", xlab ="FIA 10 Hr Fuels (kg/m -2)")
tenhourlm <- lm(FuelsWest$FIA_10.hour.kg.m.2~FuelsWest$fivex_10.hour.kg.m.2)
abline(tenhourlm)

hundhour<-plot(FuelsWest$FIA_100.hour.kg.m.2, FuelsWest$fivex_100.hour.kg.m.2, xlim = c(0,20), ylim= c(0,1), ylab = "LANDFIRE 100 Hr Fuels (median kg/m -2)", xlab =" FIA 100 Hr Fuels (kg/m -2)")
hundhourlm <- lm(FuelsWest$FIA_100.hour.kg.m.2~FuelsWest$fivex_100.hour.kg.m.2)
abline(hundhourlm)

logs<-plot(FuelsWest$FIA_Logs.kg.m.2, FuelsWest$fivex_Logs.kg.m.2, xlim = c(0,45), ylim= c(0,11), ylab = "LANDFIRE 1000 Hr Fuels (median kg/m -2)", xlab =" FIA 1000Hr Fuels (kg/m -2)")
